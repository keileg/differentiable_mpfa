...
class CommonModel:
    ...
    def __init__(self, params: Dict):
        ...
        self.rock = Rock()
        self.fluid = pp.UnitFluid()
        # Storage for analysis
        self._solutions = []
        self._residuals = []
        self._permeability_errors_nd = []
        self._permeability_errors_frac = []
    ...
    def _flux(self, subdomains: List[pp.Grid]) -> pp.ad.Operator:
        ...
        tpfa = pp.ad.TpfaAd(key, subdomains)
        self._ad.dummy_eq_for_discretization_tpfa = tpfa.flux * p
    ...
    def after_newton_iteration(self, solution_vector: np.ndarray) -> None:
        ...
        for sd, data in self.mdg.subdomains(return_data=True):
            ...
            if hasattr(self, "_porosity"):
                data[pp.STATE]["porosity"] = self._porosity(sd)
            perm_errors = np.inf
    ...
    def solve_linear_system(self) -> np.ndarray:
        """Solve linear system, keeping track of residual value.

        Returns:
            Solution update array.

        """
        dx = super().solve_linear_system()

        # EK: Doesn't this lag by one iteration? Call assemble?
        ...
        if hasattr(self, "p_analytical"):
            ...
            error = 42
        logger.info(f"Error {error}, b {np.linalg.norm(b)} and dx {np.linalg.norm(dx)}")
    ...
    def after_newton_convergence(
        ...
        Args:
            solution: Current solution vector.
            errors: Error vector for this time step.
...
class NonlinearIncompressibleFlow(CommonModel, pp.IncompressibleFlow):
    ...
    def _export_names(self) -> list[str]:
        ...
        Returns:
            list of strings identifying the data[pp.STATE] items (e.g. variables) to be
                exported.
        """
        return [self.variable]

    def _permeability(self, sd: pp.Grid) -> np.ndarray:
        """Permeability as function of pressure.
...
class Chemistry(NonlinearIncompressibleFlow):
    ...
    def _set_parameters(self) -> None:
        ...
        Order could be important, depending on implementation - flow parameters might depend
        on chemistry.
        """
    ...
    def _porosity(self, sd):
        ...
        phi = phi_0 - concentration_c
        return phi

    def _permeability_function_ad(self, concentration_c: pp.ad.Ad_array):
        phi = self._porosity_ad(concentration_c).evaluate(self.dof_manager)
        permeability = self._power_law_permeability(phi)
        if self._nonlinear_iteration < 0:
            permeability = 0 * permeability
        return permeability

    def _porosity_ad(self, concentration_c):
    ...
    def _assign_flow_equations(self):
        ...
        mass_discr = pp.ad.MassMatrixAd(self.c_parameter_key, subdomains)
        d_phi = self._porosity_ad(var_c) - self._porosity_ad(var_c.previous_timestep())
        accumulation_term = mass_discr.mass * d_phi / self._ad.time_step
    ...
    def _precipitate_mass_equation(
        ...
        accumulation = (self._porosity_ad(var_c) * c_increment) / self._ad.time_step
    ...
    def _reaction_term(
        ...
        rate = r_0 * (equilibrium_constant_inv * self._ad.component_a - 1)
        return rate
...
class BiotNonlinearTpfa(CommonModel, pp.ContactMechanicsBiot):
    ...
    def _stiffness_tensor(self, sd: pp.Grid) -> pp.FourthOrderTensor:
        ...
        Args:
            sd: Matrix subdomain grid.

        Returns:
            pp.FourthOrderTensor: Cell-wise representation of the stress tensor.
    ...
    def _fluid_flux(self, subdomains: List[pp.Grid]) -> pp.ad.Operator:
        ...
        self._ad.dummy_eq_for_discretization = base_discr.flux * p
        tpfa = pp.ad.TpfaAd(self.scalar_parameter_key, subdomains)
