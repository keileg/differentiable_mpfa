...
def extract_line_solutions(model):
    if not hasattr(model, "iteration_permeability"):
        ...
        model.iteration_c = []
    for g, d in model.gb:
        cell_inds = np.logical_and(np.isclose(g.cell_centers[2], 0.5), np.isclose(g.cell_centers[1], 0.5))
        if np.any(cell_inds):
            model.iteration_permeability.append(model._permeability(g)[cell_inds])
            if hasattr(model, "variable"):
                model.iteration_pressure.append(d[pp.STATE][pp.ITERATE][model.variable][cell_inds])
            else:
                model.iteration_pressure.append(d[pp.STATE][pp.ITERATE][model.scalar_variable][cell_inds])
            if hasattr(model, "variable"):
                model.iteration_c.append(d[pp.STATE][pp.ITERATE][model.component_c_variable][cell_inds])
...
def plot_line_solutions(m_nonlin, m_lin, color=None, model_type="both", fname=None):
    ...
    if model_type != "linear":
        for i, perms in enumerate(vals_nonl):
            ...
            if difference:
                plt.plot(
                    x, perms - vals_nonl[-1], label="Iteration " + str(i), ls="--", color=color, marker="x",
                )
            else:
                plt.plot(
                    x, perms, label="Iteration "+str(i), ls="--", color=color, marker="x",
                )
                plt.plot(x, vals_nonl[-1], label="Converged", ls=":", color=color, marker="x")
            ...
            plt.legend(title=m_lin.params.get("legend_title", None), loc='upper right')
            #if fname is None:
            fname = "images/iterations/permeabilities_nonlinear" + m_nonlin.params["plotting_file_name"]
    ...
    if model_type != "nonlinear":
        ...
        for i, perms in enumerate(vals_lin):
            ...
            if difference:
                plt.plot(
                    x, perms - vals_lin[-1], label="Iteration " + str(i), ls="--", color=color, marker="x",
                )
            else:
                plt.plot(x, perms, label="Iteration "+str(i), ls="-", color=color, marker="+")
                plt.plot(x, vals_lin[-1], label="Converged", ls=":", color=color, marker="x")
            ax = plt.gca()
            ax.set_xlabel("x coordinate")
            ax.set_ylabel("K")
            plt.ylim(c_min, c_max)
            ...
            plt.legend(title=m_lin.params.get("legend_title", None), loc='upper right')
            # if fname is None:
            fname = "images/iterations/permeabilities_linear" + m_nonlin.params["plotting_file_name"]
            plt.savefig(fname=fname+str(i))
